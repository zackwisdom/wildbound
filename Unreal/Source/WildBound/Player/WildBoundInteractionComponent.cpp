...
namespace
{
	...
	bool ParseDroppedItem(const AActor& Actor, FName& OutItemId, int32& OutQuantity)
	{
		...
		for (const FName& Tag : Actor.Tags)
		{
			...
			if (TagString.StartsWith(DroppedItemPrefix)) OutItemId = FName(*TagString.RightChop(DroppedItemPrefix.Len()));
			else if (TagString.StartsWith(DroppedQuantityPrefix)) OutQuantity = FCString::Atoi(*TagString.RightChop(DroppedQuantityPrefix.Len()));
		}
	...
	bool CanInventoryFit(const UWildBoundInventoryComponent& Inventory, FName ItemId, int32 Quantity)
	{
		...
		{
			if (Stack.ItemId == ItemId) Capacity += FMath::Max(0, Inventory.DefaultMaxStackSize - Stack.Quantity);
		}
	...
	FString GetContainerTypeName(const AActor& Container)
	{
		...
		if (Container.ActorHasTag(DumpsterContainerTag)) return TEXT("DUMPSTER");
		if (Container.ActorHasTag(CoolerContainerTag)) return TEXT("COOLER");
		if (Container.ActorHasTag(CabinetContainerTag)) return TEXT("CABINET");
		if (Container.ActorHasTag(CrateContainerTag)) return TEXT("CRATE");
	...
	FName RollLootItem(const AActor& Container, FRandomStream& Random)
	{
		...
		{
			if (Roll < 30) return MedicalItemId; if (Roll < 45) return ChemicalsItemId; if (Roll < 60) return ClothItemId;
			if (Roll < 74) return PlasticItemId; if (Roll < 84) return WaterItemId; if (Roll < 94) return AdhesiveItemId; return ElectronicsItemId;
		}
		if (Container.ActorHasTag(MarketPoolTag))
		{
			if (Roll < 34) return FoodItemId; if (Roll < 59) return WaterItemId; if (Roll < 73) return PlasticItemId;
			if (Roll < 83) return ClothItemId; if (Roll < 93) return AdhesiveItemId; return BatteryItemId;
		}
		if (Container.ActorHasTag(ResidentialPoolTag))
		{
			if (Roll < 18) return FoodItemId; if (Roll < 33) return WaterItemId; if (Roll < 48) return ClothItemId; if (Roll < 61) return WoodItemId;
			if (Roll < 72) return PlasticItemId; if (Roll < 81) return AdhesiveItemId; if (Roll < 89) return BatteryItemId; if (Roll < 96) return ElectronicsItemId; return FlashlightItemId;
		}
		...
		{
			if (Roll < 25) return ElectronicsItemId; if (Roll < 44) return WireItemId; if (Roll < 59) return BatteryItemId; if (Roll < 73) return PlasticItemId;
			if (Roll < 84) return ClothItemId; if (Roll < 94) return AdhesiveItemId; return MedicalItemId;
		}
		if (Roll < 18) return ScrapItemId; if (Roll < 32) return FoodItemId; if (Roll < 45) return WaterItemId; if (Roll < 58) return ClothItemId;
		if (Roll < 70) return PlasticItemId; if (Roll < 80) return WoodItemId; if (Roll < 89) return AdhesiveItemId; if (Roll < 96) return WireItemId; return BatteryItemId;
	}
	...
	void AddQualityBonusLoot(const AActor& Container, int32 QualityTier, FRandomStream& Random, TMap<FName, int32>& Grants)
	{
		...
		if ((Container.ActorHasTag(ResidentialPoolTag) || Container.ActorHasTag(MarketPoolTag)) && Random.FRand() < 0.04f + TierBonus) Grants.FindOrAdd(CanteenItemId) += 1;
		if (Container.ActorHasTag(IndustrialPoolTag) && Random.FRand() < 0.02f + TierBonus * 0.60f) Grants.FindOrAdd(UtilityBeltItemId) += 1;

		if (QualityTier >= 3)
		{
			if (Container.ActorHasTag(MedicalPoolTag)) { Grants.FindOrAdd(TraumaKitItemId) += 1; Grants.FindOrAdd(RadTreatmentItemId) += 2; }
			else if (Container.ActorHasTag(IndustrialPoolTag)) { Grants.FindOrAdd(UtilityBeltItemId) += 1; Grants.FindOrAdd(CrowbarItemId) += 1; }
			else if (Container.ActorHasTag(CivicPoolTag)) { Grants.FindOrAdd(FilterMaskItemId) += 1; Grants.FindOrAdd(ElectronicsItemId) += 2; }
			else if (Container.ActorHasTag(ResidentialPoolTag) || Container.ActorHasTag(MarketPoolTag)) { Grants.FindOrAdd(CanteenItemId) += 1; Grants.FindOrAdd(FlashlightItemId) += 1; }
			else Grants.FindOrAdd(ReinforcedBackpackItemId) += 1;
		}
	}
}
...
void UWildBoundInteractionComponent::SetContextPrompt(const FString& Prompt, int32 Priority)
{
	...
	if (!World || Prompt.IsEmpty()) return;
...
{
	return GetOwner() ? GetOwner()->FindComponentByClass<UWildBoundInventoryComponent>() : nullptr;
}

const TArray<FWildBoundContainerLootEntry>* UWildBoundInteractionComponent::GetOpenContainerLoot() const
{
	return OpenContainerActor.IsValid() ? ContainerLootByActor.Find(OpenContainerActor) : nullptr;
}

FString UWildBoundInteractionComponent::GetOpenContainerName() const
{
	return OpenContainerActor.IsValid() ? GetContainerTypeName(*OpenContainerActor.Get()) : TEXT("CONTAINER");
}

FString UWildBoundInteractionComponent::GetOpenContainerQualityName() const
{
	return OpenContainerActor.IsValid() ? GetContainerQualityName(*OpenContainerActor.Get()) : TEXT("COMMON");
}

int32 UWildBoundInteractionComponent::GetOpenContainerQualityTier() const
{
	return OpenContainerActor.IsValid() ? GetContainerQualityTier(*OpenContainerActor.Get()) : 0;
}

void UWildBoundInteractionComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (bHasWorldInteraction)
	{
		...
		if (PlayerController->WasInputKeyJustPressed(EKeys::E)) TryInteract(TargetActor);
		return;
	}

	if (PlayerController->WasInputKeyJustPressed(EKeys::E)) TryUseSelectedHotbarItem();
}

void UWildBoundInteractionComponent::HandleHotbarSelection(APlayerController& PlayerController)
{
	if (PlayerController.WasInputKeyJustPressed(EKeys::One)) SelectedHotbarSlot = 0;
	else if (PlayerController.WasInputKeyJustPressed(EKeys::Two)) SelectedHotbarSlot = 1;
	else if (PlayerController.WasInputKeyJustPressed(EKeys::Three)) SelectedHotbarSlot = 2;
	else if (PlayerController.WasInputKeyJustPressed(EKeys::MouseScrollDown)) SelectedHotbarSlot = (SelectedHotbarSlot + 1) % HotbarSlotCount;
	else if (PlayerController.WasInputKeyJustPressed(EKeys::MouseScrollUp)) SelectedHotbarSlot = (SelectedHotbarSlot + HotbarSlotCount - 1) % HotbarSlotCount;
}
...
void UWildBoundInteractionComponent::TryUseSelectedHotbarItem()
{
	...
	if (ItemId.IsNone())
	{
		if (GEngine) GEngine->AddOnScreenDebugMessage(91003, 1.5f, FColor(185,185,175), TEXT("That hotbar slot is empty."));
		return;
	}
...
FString UWildBoundInteractionComponent::GetInteractionPrompt(const AActor* TargetActor) const
{
	...
	if (TargetActor->ActorHasTag(PryLockedTag))
	{
		...
		if (TargetActor->ActorHasTag(PryAccessTag)) return bHasCrowbar ? TEXT("Pry open maintenance gate") : TEXT("Locked gate - crowbar required");
		const FString Quality = GetContainerQualityName(*TargetActor);
		return bHasCrowbar ? FString::Printf(TEXT("Pry open [%s] locked cache"), *Quality) : FString::Printf(TEXT("[%s] cache locked - crowbar required"), *Quality);
	}

	if (TargetActor->ActorHasTag(ContainerTag))
	{
		const bool bOpened = TargetActor->ActorHasTag(SearchedContainerTag);
		return FString::Printf(TEXT("[%s] %s %s"), *GetContainerQualityName(*TargetActor), bOpened ? TEXT("Open") : TEXT("Search"), *GetContainerTypeName(*TargetActor).ToLower());
	}
...
void UWildBoundInteractionComponent::TryInteract(AActor* TargetActor)
{
	...
	if (TargetActor->ActorHasTag(DroppedItemTag))
	{
		...
		if (!Inventory || !ParseDroppedItem(*TargetActor, ItemId, Quantity)) return;
		if (!CanInventoryFit(*Inventory, ItemId, Quantity) || !Inventory->AddItem(ItemId, Quantity)) { if (GEngine) GEngine->AddOnScreenDebugMessage(91002,1.8f,FColor(220,145,115),TEXT("Not enough inventory space.")); return; }
		if (GEngine) GEngine->AddOnScreenDebugMessage(91002,2.1f,GetRarityColor(Inventory->GetItemRarityTier(ItemId)),FString::Printf(TEXT("Picked up [%s] %s x%d"),*Inventory->GetItemRarityName(ItemId),*Inventory->GetItemDisplayName(ItemId),Quantity));
		TargetActor->Destroy(); return;
	}
	if (TargetActor->ActorHasTag(PryLockedTag)) { TryPryTarget(TargetActor); return; }
	if (TargetActor->ActorHasTag(ContainerTag)) { SearchLootContainer(TargetActor); return; }
	if (TargetActor->ActorHasTag(SupplyTag))
	{
		UWildBoundInventoryComponent* Inventory = GetInventoryComponent(); if (!Inventory) return;
		FName ItemId = NAME_None; FName GroupTag = NAME_None; int32 Quantity = 0; FString PickupMessage;
		if (TargetActor->ActorHasTag(WaterTag)) { ItemId = WaterItemId; GroupTag = WaterGroupTag; Quantity = 4; PickupMessage = TEXT("Collected 4 bottled waters"); }
		else if (TargetActor->ActorHasTag(MedicalTag)) { ItemId = MedicalItemId; GroupTag = MedicalGroupTag; Quantity = 1; PickupMessage = TEXT("Collected first-aid kit"); }
		else if (TargetActor->ActorHasTag(FoodTag)) { ItemId = FoodItemId; GroupTag = FoodGroupTag; Quantity = 3; PickupMessage = TEXT("Collected 3 preserved food rations"); }
...
void UWildBoundInteractionComponent::TryPryTarget(AActor* TargetActor)
{
	if (!TargetActor) return;
	const UWildBoundInventoryComponent* Inventory = GetInventoryComponent();
	if (!Inventory || !Inventory->HasItem(CrowbarItemId,1)) { if (GEngine) GEngine->AddOnScreenDebugMessage(91041,1.8f,FColor(220,145,100),TEXT("You need a crowbar to force this open.")); return; }
...
{
	if (!TargetActor) return;
	TWeakObjectPtr<AActor> Key(TargetActor);
	...
	RefreshContainerInteractableState(TargetActor);
	OpenLootWindow(*TargetActor);
}

void UWildBoundInteractionComponent::GenerateContainerLoot(AActor& TargetActor, TArray<FWildBoundContainerLootEntry>& OutLoot)
{
	OutLoot.Reset();
	UWildBoundInventoryComponent* Inventory = GetInventoryComponent();
	if (!Inventory) return;
	...
	const int32 LocationSeed = HashCombine(GetTypeHash(FMath::RoundToInt(Location.X)), HashCombine(GetTypeHash(FMath::RoundToInt(Location.Y)), GetTypeHash(FMath::RoundToInt(Location.Z))));
	FRandomStream Random(HashCombine(LocationSeed, FMath::Rand()));

	float EmptyChance = 0.18f; int32 MinRolls = 2; int32 MaxRolls = 4;
	if (QualityTier == 1) { EmptyChance = 0.09f; MinRolls = 3; MaxRolls = 4; }
	else if (QualityTier == 2) { EmptyChance = 0.03f; MinRolls = 3; MaxRolls = 5; }
	else if (QualityTier >= 3) { EmptyChance = 0.0f; MinRolls = 4; MaxRolls = 6; }
	if (TargetActor.ActorHasTag(MedicalPoolTag) || TargetActor.ActorHasTag(IndustrialPoolTag)) EmptyChance = FMath::Max(0.0f, EmptyChance - 0.03f);
	if (Random.FRand() < EmptyChance) return;
	...
	for (int32 RollIndex = 0; RollIndex < Rolls; ++RollIndex)
	{
		...
		if (QualityTier >= 2 && Inventory->GetItemRarityTier(ItemId) <= 1 && Random.FRand() < 0.55f) ++Quantity;
		Grants.FindOrAdd(ItemId) += Quantity;
	}
	AddQualityBonusLoot(TargetActor, QualityTier, Random, Grants);

	Grants.KeySort([Inventory](const FName& A, const FName& B)
	{
		const int32 RarityA = Inventory->GetItemRarityTier(A); const int32 RarityB = Inventory->GetItemRarityTier(B);
		return RarityA == RarityB ? Inventory->GetItemDisplayName(A) < Inventory->GetItemDisplayName(B) : RarityA > RarityB;
	});
	for (const TPair<FName,int32>& Grant : Grants)
	{
		if (Grant.Value > 0) { FWildBoundContainerLootEntry Entry; Entry.ItemId = Grant.Key; Entry.Quantity = Grant.Value; OutLoot.Add(Entry); }
	}
}

void UWildBoundInteractionComponent::EnsureLootWidget()
{
	if (LootViewportRoot.IsValid())
	{
		if (LootWidget.IsValid()) LootWidget->SetInteractionComponent(this);
		return;
	}
	...
	[
		...
		[
			...
			+ SOverlay::Slot().HAlign(HAlign_Fill).VAlign(VAlign_Fill)
			[
				SNew(SBorder).BorderBackgroundColor(FLinearColor(0.0f,0.0f,0.0f,0.38f))
			]
			+ SOverlay::Slot().HAlign(HAlign_Center).VAlign(VAlign_Center).Padding(FMargin(24.0f))
			[
				SNew(SBox).WidthOverride(860.0f)
				[
					SAssignNew(LootWidget, SWildBoundLootWidget).InteractionComponent(this)
				]
			]
		]
	];
	LootViewportRoot = Overlay;
	LootViewportRoot->SetVisibility(EVisibility::Collapsed);
	GEngine->GameViewport->AddViewportWidgetContent(LootViewportRoot.ToSharedRef(), 135);
}

void UWildBoundInteractionComponent::OpenLootWindow(AActor& TargetActor)
{
	OpenContainerActor = &TargetActor;
	EnsureLootWidget();
	bLootWindowOpen = LootViewportRoot.IsValid();
	if (LootViewportRoot.IsValid()) LootViewportRoot->SetVisibility(bLootWindowOpen ? EVisibility::Visible : EVisibility::Collapsed);

	APawn* Pawn = Cast<APawn>(GetOwner());
	APlayerController* PC = Pawn ? Cast<APlayerController>(Pawn->GetController()) : nullptr;
	if (PC && bLootWindowOpen)
	{
		PC->SetIgnoreMoveInput(true); PC->SetIgnoreLookInput(true); PC->bShowMouseCursor = true;
		FInputModeGameAndUI InputMode; InputMode.SetHideCursorDuringCapture(false); InputMode.SetLockMouseToViewportBehavior(EMouseLockMode::DoNotLock); PC->SetInputMode(InputMode);
	}
}

void UWildBoundInteractionComponent::CloseLootWindow()
{
	if (!bLootWindowOpen && !OpenContainerActor.IsValid()) return;
	AActor* PreviousContainer = OpenContainerActor.Get();
	bLootWindowOpen = false;
	if (LootViewportRoot.IsValid()) LootViewportRoot->SetVisibility(EVisibility::Collapsed);
	if (PreviousContainer) RefreshContainerInteractableState(PreviousContainer);
	OpenContainerActor.Reset();
...
void UWildBoundInteractionComponent::RemoveLootWidget()
{
	if (LootViewportRoot.IsValid() && GEngine && GEngine->GameViewport) GEngine->GameViewport->RemoveViewportWidgetContent(LootViewportRoot.ToSharedRef());
	LootWidget.Reset(); LootViewportRoot.Reset(); bLootWindowOpen = false; OpenContainerActor.Reset();
}

bool UWildBoundInteractionComponent::TakeLootEntry(int32 EntryIndex, bool bTakeWholeStack)
{
	UWildBoundInventoryComponent* Inventory = GetInventoryComponent();
	TArray<FWildBoundContainerLootEntry>* Loot = OpenContainerActor.IsValid() ? ContainerLootByActor.Find(OpenContainerActor) : nullptr;
	if (!Inventory || !Loot || !Loot->IsValidIndex(EntryIndex)) return false;

	FWildBoundContainerLootEntry& Entry = (*Loot)[EntryIndex];
	const int32 QuantityToTake = bTakeWholeStack ? Entry.Quantity : 1;
	if (QuantityToTake <= 0 || !CanInventoryFit(*Inventory, Entry.ItemId, QuantityToTake) || !Inventory->AddItem(Entry.ItemId, QuantityToTake))
	{
		if (GEngine) GEngine->AddOnScreenDebugMessage(91052,1.8f,FColor(220,145,115),TEXT("Not enough backpack space for that loot."));
		return false;
	}

	if (GEngine) GEngine->AddOnScreenDebugMessage(91052,1.8f,GetRarityColor(Inventory->GetItemRarityTier(Entry.ItemId)),FString::Printf(TEXT("Took [%s] %s x%d"),*Inventory->GetItemRarityName(Entry.ItemId),*Inventory->GetItemDisplayName(Entry.ItemId),QuantityToTake));
	Entry.Quantity -= QuantityToTake;
	if (Entry.Quantity <= 0) Loot->RemoveAt(EntryIndex);
	RefreshContainerInteractableState(OpenContainerActor.Get());
	return true;
}

void UWildBoundInteractionComponent::TakeAllContainerLoot()
{
	TArray<FWildBoundContainerLootEntry>* Loot = OpenContainerActor.IsValid() ? ContainerLootByActor.Find(OpenContainerActor) : nullptr;
	if (!Loot) return;

	bool bTookAnything = false;
	for (int32 Index = Loot->Num() - 1; Index >= 0; --Index)
	{
		bTookAnything |= TakeLootEntry(Index, true);
	}
	if (!bTookAnything && GEngine) GEngine->AddOnScreenDebugMessage(91052,1.6f,FColor(180,180,170),TEXT("Nothing could be moved to your backpack."));
}
...
{
	UWorld* World = GetWorld(); if (!World || GroupTag.IsNone()) return;
